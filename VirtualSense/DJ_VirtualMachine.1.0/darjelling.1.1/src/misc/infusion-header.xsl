				...
				<xsl:for-each select="classdef">
					...
					<div class="indent">
						...
						<div>
							...
							(
							<xsl:value-of select="@superclass.infusion" />
							.
							<xsl:value-of select="@superclass.entity_id" />
							)
						</div>
						...
						<xsl:if test="count(method) &gt; 0">
							...
							<ul class="methodlist">
								<xsl:for-each select="method">
									<li>
										def(
										<xsl:value-of select="@methoddef.infusion" />
										.
										<xsl:value-of select="@methoddef.entity_id" />)
										-&gt;
										impl(
										<xsl:value-of select="@methodimpl.infusion" />
										.
										<xsl:value-of select="@methodimpl.entity_id" />
										)
									</li>
								</xsl:for-each>
							</ul>
			...
			<div class="indent">
				<xsl:for-each select="methodimpl">
					...
					<div class="indent">
						...
						<div>
							Parent class: (
							<xsl:value-of select="@parentclass.infusion" />
							.
							<xsl:value-of select="@parentclass.entity_id" />
							)
						</div>
					</div>
				</xsl:for-each>
	...
	<xsl:template match="/dih">
		<html>
			...
			<body>
				<h1>
					Infusion
					<xsl:value-of select="infusion/header/@name" />
				</h1>
