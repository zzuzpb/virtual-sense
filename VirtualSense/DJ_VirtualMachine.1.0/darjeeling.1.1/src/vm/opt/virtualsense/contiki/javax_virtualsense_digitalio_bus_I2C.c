...
void javax_virtualsense_digitalio_bus_I2C_void__start(){
	i2c_start();
}
void javax_virtualsense_digitalio_bus_I2C_void__stop(){
	i2c_stop();
}
void javax_virtualsense_digitalio_bus_I2C_byte__read_boolean(){
	uint16_t ack_send = dj_exec_stackPopShort();
	uint16_t ret = i2c_read((int)ack_send);
	dj_exec_stackPushShort(ret);
}
void javax_virtualsense_digitalio_bus_I2C_boolean__write_byte(){
